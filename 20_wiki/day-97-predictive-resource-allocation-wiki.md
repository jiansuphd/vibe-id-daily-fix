---
tags:
  - wiki
  - vibe-coding
  - phase-7
  - day-97
---

# Day 97: Predictive Resource Allocation - Wiki

## Core Concept

Use project history to predict the resource requirements of the next course project before scoping begins. Historical velocity data is your best planning tool. An agent that reads your past project logs estimates hours, identifies high-risk phases, and builds a resource forecast - before the kickoff meeting.

## ID Framework Alignment

| Dimension | Application |
|-----------|-------------|
| **Design Principle** | Predictive Resource Allocation |
| **Phase** | Phase 7: Endgame & Sustained Resilience |
| **Key Skill** | resource-allocation, prediction, planning |

## Agentic CLI Prompt

```bash
gemini "Based on my past project logs (00_meta/progress.md), estimate the resource requirements for a new 8-week online course for a 40-student CUIN section. Include risk multipliers."
```

## Key Terms

- **Predictive:** The core technique or system described in this entry.
- **Phase 7:** Endgame & Sustained Resilience - the overarching theme of Days 141–140.
- **Pointer:** The strategic framing that makes this technique generalizable beyond its immediate application.

## Backlinks

- [20 Wiki MOC](20_wiki_MOC.md)
- [Root MOC](../root_MOC.md)
- [Source Daily](../10_dailies/day-97-predictive-resource-allocation.md)
